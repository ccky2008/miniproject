...
app.post('/editRestaurant', function(req,res) {
  ...
  var sampleFile = (req.files != null) ? new Buffer(req.files.sampleFile.data).toString('base64') : null;
  var mimetype = req.files.sampleFile.mimetype;
  var rate = rate_restaurant;
  MongoClient.connect(mongourl,function(err,db) {
      console.log('Connected to mlab.com');
      assert.equal(null,err);
      editOneRestaurant(db,idToFind,restaurantname,cuisine,street,building,zipcode,long,lat,borough,username,sampleFile, mimetype, rate,function() {
          db.close();
...
function editOneRestaurant(db, criteria, name,cuisine,street,building,zipcode,log,lat,borough,username,sampleFile,mimetype, rate, callback) {
  var address = {};
  address.street = street;
  address.building = building;
  address.zipcode = zipcode;
